...
$query_edit = "SELECT * FROM karyawan WHERE id = '$id_edit'";
...
$kode = $data_edit['kode'];
$nama = $data_edit['nama'];
$sandi = $data_edit['sandi'];
$role = $data_edit['role'];
$sukses = ""; $error = "";

if (isset($_POST['simpan'])) {
    $kode_baru  = mysqli_real_escape_string($conn, $_POST['kode']);
    $nama_baru  = mysqli_real_escape_string($conn, $_POST['nama']);
    $sandi_baru = mysqli_real_escape_string($conn, $_POST['sandi']);
    $role_baru  = mysqli_real_escape_string($conn, $_POST['role']);

    if ($kode_baru && $nama_baru && $role_baru && $sandi_baru) {
        $query = "UPDATE karyawan SET kode='$kode_baru', nama='$nama_baru', sandi='$sandi_baru', role='$role_baru' WHERE id = '$id_edit'";
        $result = mysqli_query($conn, $query);

        if ($result) {
            $sukses = "Data karyawan berhasil diperbarui!";
            // Update variabel untuk ditampilkan kembali di form
            $kode = $kode_baru; $nama = $nama_baru; $sandi = $sandi_baru; $role = $role_baru;
        } else {
...
<head>
    ...
    <title>Edit Karyawan - BengCare</title>
    ...
    <div class="staff-wrap" style="background-color: transparent !important;">
        ...
        <main class="staff-main p-4 p-md-5" style="background-color: transparent !important;">
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
                <div>
                    <h1 class="panel-title text-white" style="font-size: 2.2rem;">Edit Data Karyawan</h1>
                    <p class="panel-sub text-white-50">Perbarui data login atau jabatan karyawan.</p>
                ...
                <div class="card-body p-4">
                    ...
                    <form action="" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="password" class="form-control" id="sandi" name="sandi" value="<?= htmlspecialchars($sandi); ?>" required>
                        </div>
                        <div class="mb-4">
