...
if (isset($_POST['simpan'])) {
    ...
    if ($error == "" && $id_kategori_layanan && $nama_layanan && $harga && $id_karyawan) {
        $stmt_insert = mysqli_prepare($conn, "INSERT INTO layanan (id_kategori_layanan, nama_layanan, deskripsi, estimasi_waktu, harga, id_karyawan) VALUES (?, ?, ?, ?, ?, ?)");
        mysqli_stmt_bind_param($stmt_insert, "isssii", $id_kategori_layanan, $nama_layanan, $deskripsi, $estimasi_waktu, $harga, $id_karyawan);
        $result = mysqli_stmt_execute($stmt_insert);

        if ($result) {
            $sukses = "Berhasil menambahkan layanan baru!";
            $nama_layanan = ""; $deskripsi = ""; $estimasi_waktu = ""; $harga = "";
        } else {
            $error = "Gagal menambahkan layanan!";
        }
    } elseif ($error == "") {
        $error = "Silakan lengkapi data yang wajib diisi!";
    }
}
?>
...
<head>
    ...
    <title>Tambah Layanan - BengCare</title>
    ...
    <div class="staff-wrap" style="background-color: transparent !important;">
        ...
        <main class="staff-main p-4 p-md-5" style="background-color: transparent !important;">
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
                <div>
                    <h1 class="panel-title text-white" style="font-size: 2.2rem;">Tambah Data Layanan</h1>
                    <p class="panel-sub text-white-50">Tambahkan layanan servis baru ke dalam katalog.</p>
                ...
                <div class="card-body p-4">
                    ...
                    <form action="" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" class="form-control" name="nama_layanan" placeholder="Contoh: Ganti Oli Mesin" value="<?= htmlspecialchars($nama_layanan); ?>" required>
                        </div>

                        <div class="mb-4">
                            <label class="form-label text-muted small fw-bold">DESKRIPSI</label>
                            <textarea class="form-control" name="deskripsi" rows="3" placeholder="Jelaskan detail layanan..."><?= htmlspecialchars($deskripsi); ?></textarea>
                        </div>

                        <div class="row mb-4">
                            <div class="col-md-6">
                                <label class="form-label text-muted small fw-bold">ESTIMASI WAKTU</label>
                                <input type="text" class="form-control" name="estimasi_waktu" placeholder="Contoh: 30 Menit" value="<?= htmlspecialchars($estimasi_waktu); ?>">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label text-muted small fw-bold">ESTIMASI BIAYA (Rp) <span class="text-danger">*</span></label>
                                <input type="number" min="0" class="form-control" name="harga" placeholder="75000" value="<?= htmlspecialchars($harga); ?>" required>
                            </div>
                        </div>
